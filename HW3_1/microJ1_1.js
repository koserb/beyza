  ...
  toString() {
     ...
     for (let i=0; i<this.nParams; i++){
     	if (this.paramValues[i]!= null){
	        a.push(this.vars[i]+" = "+this.paramValues[i])
        }else
        a.push(this.vars[i])
	}
...
function method()  {
    ...
    if (tok.kind == IDENT) {
    	let paramlist=identList(); //parameteres as variable with value
    
   		met.nParams = met.vars.length; 
    	for(let i =0;i<paramlist.length;i++){
    		met.paramValues.push(paramlist[i].val);
    	}
    }
